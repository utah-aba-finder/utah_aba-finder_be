...
RSpec.describe 'Providers API', type: :request do
  ...
  path '/api/v1/providers/{id}' do
    get('get provider by ID') do
      tags 'Providers'
      produces 'application/json'
      description 'Retrieves a single provider by their ID. Requires User login with authorization to access that providers info. Rate limited to 100 requests per minute.'
